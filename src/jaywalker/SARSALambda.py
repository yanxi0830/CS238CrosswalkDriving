...
class SARSA_Lambda:
    def __init__(self, env):
        ...
        self.Emat = np.zeros((position * v * 3, self.actionspace))
    ...
    def train(self):
        ...
        for i in range(self.episode):
            ...
            for j in range(self.horizon):
                ...
                delta = reward + self.gamma * self.Qmat[next_state, next_action] - self.Qmat[curstate, curaction]
                self.Emat[curstate, curaction] = self.Emat[curstate, curaction] + 1

                for s in range(self.Qmat.shape[0]):
                    for a in range(self.Qmat.shape[1]):
                        self.Qmat[s, a] = self.Qmat[s, a] + self.alpha * delta * self.Emat[s, a]
                        self.Emat[s, a] = self.gamma * self.lamb * self.Emat[s, a]
        ...
        plot(rewardperep, 'results/sarsa_lambda_traffic_returns.png')
